<script>
  ...
  function headerSelected(option) {
    let selected = { Menu: option }
  }

</script>

<nav class="relative px-4 py-4 flex justify-between items-center bg-white md:bg-white lg:bg-white">

  <a class="text-3xl font-bold leading-none" href="q">
    <svg class="h-16" alt="logo" viewBox="0 0 10240 10240">
      {@html chapigblacklogo}
    </a>

  <div class="lg:hidden md:hidden">
    <button on:click={toggle} class="navbar-burger flex items-center text-black bg-transparent rounded p-3">
  ...
  <ul class="flex flex-col md:flex-row md:mx-6">
    <li><Link to="/" class="text-md text-black font-bold hover:text-gray-500 md:mx-4 md:my-0">Inicio</Link></li>
    <li><Link to="services" class="text-md text-black  hover:text-gray-500 md:mx-4 md:my-0" href="q">Servicios</Link></li>
    <li><Link to="cv" class="text-md text-black hover:text-gray-500 md:mx-4 md:my-0" href="q">Currículum</Link></li>
    <li><Link to="contact" class="text-md text-black hover:bg-gray-900 hover:text-white transition duration-200 md:mx-4 md:my-0 py-2 px-4 rounded-md bg-yellow-400">Contáctame</Link></li>
  </ul>
  </div>
</nav>

{#if menu.open}
  <div class="navbar-menu relative z-50" transition:fade="{{delay: 250, duration: 300}}">
    <div class="navbar-backdrop fixed inset-0 bg-gray-900 opacity-25"></div>
    <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 overflow-y-auto" style="background-color: #1a1a1b;">
      <div class="flex items-center mb-8">
        <a class="mr-auto text-3xl font-bold leading-none" href="q">
          <img src="white-chapig.svg" class="h-8 pt-2" alt="logo">
        </a>
      ...
      <div>
        <ul>
          <li class="mb-1">
            <Link to="/" class="block p-4 text-sm text-white hover:text-yellow-400">Inicio</Link>
          </li>
          <li class="mb-1">
            <Link to="services" class="block p-4 text-sm text-white hover:text-yellow-400">Servicios</Link>
          </li>
          <li class="mb-1">
            <Link to="cv" class="block p-4 text-sm text-white hover:text-yellow-400">Currículum</Link>
          </li>
        </ul>
      </div>
      <div class="mt-auto">
        <div class="pt-6">
          <Link to="contact" class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-white">Contáctame</Link>
        </div>
